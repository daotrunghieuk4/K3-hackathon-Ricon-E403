...
-- PostgreSQL CHECK constraints cannot inspect another table, so role-sensitive
-- foreign keys are enforced here.
...
begin
  if tg_table_name = 'classrooms' then
    select role into target_role from app_users where id = new.owner_admin_id;
    if target_role is distinct from 'admin' then
      raise exception 'classrooms.owner_admin_id must reference an admin';
    end if;
  elsif tg_table_name = 'classroom_enrollments' then
    select role into target_role from app_users where id = new.student_id;
    if target_role is distinct from 'user' then
      raise exception 'classroom_enrollments.student_id must reference a user';
    end if;
  elsif tg_table_name = 'ai_provider_configs' then
    select role into target_role from app_users where id = new.owner_admin_id;
    if target_role is distinct from 'admin' then
      raise exception 'ai_provider_configs.owner_admin_id must reference an admin';
    end if;
  elsif tg_table_name = 'quizzes' and new.owner_student_id is not null then
    select role into target_role from app_users where id = new.owner_student_id;
    if target_role is distinct from 'user' then
      raise exception 'quizzes.owner_student_id must reference a user';
    end if;
  end if;
...
create trigger quizzes_enforce_owner_role
before insert or update of owner_student_id on quizzes
for each row execute function enforce_vlearn_user_roles();
...
begin
  target_question_id =
    case
      when tg_table_name = 'questions' then old.id
  ...
  if exists (
    ...
    where
      ...
      and qz.status in ('published', 'closed')
  ) then
    raise exception
...
before update or delete on question_options
for each row execute function prevent_published_question_mutation();
...
select
  ...
  round(avg(qa.total_score / nullif(qa.max_score, 0) * 100), 2)
